...
def build_pptx(pptx_content):
    ...
    FileUtil.copy_directory(PPTX_BASE, working_dir)
    
    no_slides = len(pptx_content)
    
    content_types = build_content_types(no_slides)
    FileUtil.write_file_content(working_dir + '/[Content_Types].xml', content_types)
    
    presentation = build_presentation(no_slides)
    FileUtil.write_file_content(working_dir + '/ppt/presentation.xml', presentation)
    
    presentation_rels = build_presentation_rels(no_slides)
    FileUtil.write_file_content(working_dir + '/ppt/_rels/presentation.xml.rels', presentation_rels)
    
    for idx, val in enumerate(pptx_content):
        slide = build_slide(val)
        slide_filename = working_dir + '/ppt/slides/slide%d.xml' % (idx + 1)
        FileUtil.write_file_content(slide_filename, slide)
        
        FileUtil.copy_file(PPTX_TEMPLATES + '/ppt/slides/_rels/slide.xml.rels', working_dir + '/ppt/slides/_rels/slide%d.xml.rels' % (idx + 1))
        
    FileUtil.remove_file(working_dir + '/ppt/_rels/.keep')
    FileUtil.remove_file(working_dir + '/ppt/slides/_rels/.keep')
    
    FileUtil.zip_directory(working_dir, output_filename)
    ...
    FileUtil.rename_file(output_filename + '.zip', PPTX_FILES + pptx_filename)
...
def remove_old_pptx():
    ...
    for filename in FileUtil.list_files_in_dir(PPTX_FILES):
        if filename != PPTX_ERROR_FILE:
            fullpath = PPTX_FILES + filename
            file_created = FileUtil.get_file_created_time(fullpath)
            
            if curr_time - file_created > (60 * 60 * 24):
                print('Remove ' + filename)
                FileUtil.remove_file(fullpath)
...
def build_slide(pptx_content):
    ...
    slide_content = FileUtil.read_file_content(slide_filename)
...
if __name__ == '__main__':
    pptx_content = {
        ...
        'version_list': [1, 2],
        'text': u'1. 여호와는 나의 빛이요 나의 구원이시니 내가 누구를 두려워하리요 여호와는 내 생명의 능력이시니 내가 누구를 무서워하리요',
    }
    
    slide = build_slide(pptx_content)
    FileUtil.write_file_content('./temp_slide1.xml', slide)
